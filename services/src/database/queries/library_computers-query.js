// CREATE
const CREATE_LIBRARY_COMPUTER = `
  INSERT INTO library_computers (computer_number, status)
  VALUES (?, ?)
`;

// READ - Get all records
const FIND_ALL_LIBRARY_COMPUTER = `
  SELECT
    lc.id,
    lc.computer_number,
    lc.status,
    lc.created_at,
    lc.updated_at
  FROM library_computers AS lc
  ORDER BY lc.id DESC
`;

// READ - Get record by ID
const FIND_LIBRARY_COMPUTER_BY_ID = `
  SELECT
    lc.id,
    lc.computer_number,
    lc.status,
    lc.created_at,
    lc.updated_at
  FROM library_computers AS lc
  WHERE lc.id = ?
`;

// UPDATE
const UPDATE_LIBRARY_COMPUTER = `
  UPDATE library_computers AS lc
  SET
    lc.computer_number = ?,
    lc.status = ?
  WHERE lc.id = ?
`;

// DELETE
const DELETE_LIBRARY_COMPUTER = `
  DELETE FROM library_computers
  WHERE id = ?
`;

module.exports = {
  CREATE_LIBRARY_COMPUTER,
  FIND_ALL_LIBRARY_COMPUTER,
  FIND_LIBRARY_COMPUTER_BY_ID,
  UPDATE_LIBRARY_COMPUTER,
  DELETE_LIBRARY_COMPUTER,
};
